...
def submit_payment(user):
    ...
    if not plan:
        return jsonify({"detail": "Plan nahi mila"}), 400
    if not utr_number:
        return jsonify({"detail": "UTR / Transaction number daalein"}), 400

    screenshot = request.files.get("screenshot")
    filename = save_uploaded_image(screenshot, current_app.config["SCREENSHOT_FOLDER"])
    if not filename:
        return jsonify({"detail": "Payment screenshot upload karein (jpg/png)"}), 400
    ...
    db.session.add(pr)
    db.session.commit()
    return jsonify({"message": "Payment request submit ho gaya! Admin verify karne ke "
                              "baad aapka plan activate ho jaayega.",
